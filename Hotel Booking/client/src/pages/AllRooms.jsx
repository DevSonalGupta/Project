    ...
    <div className="flex flex-col-reverse lg:flex-row items-start justify-between pt-28 md:pt-35 px-4 md:px-16 lg:px-24">
      ...
      <div>
        ...
        {roomsDummyData.map((rooms) => (
          <div
            key={rooms._id}
            className="flex flex-xol md:flex-row items-start py-10 gap-6 border-b border-gray-300 last:pb-30 last:border-0"
          >
            <img
              onClick={() => {
      ...
      {/* Filters */}
      <div className="bg-white w-80 border border-gray-300 text-gray-600 max-lg:mg:-8 min-lh:mt-16">
        <div
          className={`flex items-center justify-between px-5 py-2.5 min-lg:birder-b border-gray-300 ${
            openFilter && "border-b"
          }`}
        >
        ...
        >
          ...
          <div className="px-5 pt-5">
            ...
            {sortOption.map((option, index) => (
              <RadioBox key={index} label={option} />
            ))}
          </div>
        </div>
      </div>
