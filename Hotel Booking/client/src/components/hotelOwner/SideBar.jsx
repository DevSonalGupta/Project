...
const SideBar = () => {
  ...
  return (
    <div className="md:w-64 w-64 border-r h-full text-base border-gray-300 pt-4 flex flex-col transition-all duration-300">
      {sidebarLinks.map((item, index) => (
        ...
        >
          ...
          <p className="md:block hidden text-cursor">{item.name}</p>
        </NavLink>
      ))}
    </div>
  );
