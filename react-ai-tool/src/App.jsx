  ...
  return (
    <div className={darkMode == "dark" ? "dark" : "light"}>
      <div className="grid grid-cols-5 h-screen text-center">
        <select
          onClick={(event) => setDarkMode(event.target.value)}
        ...
        <div className="col-span-4 p-10">
          <h1 className="text-5xl bg-clip-text text-transparent bg-gradient-to-r from-pink-700 to-violet-700">
          ...
          <div
            ref={scrollToAns}
            className="container h-150 overflow-auto no-scrollbar"
          >
            <div className="dark:text-zinc-300 text-zinc-800">
              <ul>
                {result.map((item, index) => (
                  <QuesAns item={item} index={index} key={index} />
                ))}
              </ul>
            </div>
          </div>
